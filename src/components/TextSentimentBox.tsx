...
const TextSentimentBox = ({ onResult }: { onResult?: (r: SentimentResult) => void }) => {
  ...
  const analyze = async () => {
    ...
    const r: SentimentResult = { sentiment: s, score: 0.55 + Math.random() * 0.43, keywords: ['stress', 'work', 'tired'].slice(0, 2) };
    setResult(r);
    onResult?.(r);
    setLoading(false);
  };
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        ...
        <Button size="sm" className="btn-primary" onClick={analyze} disabled={!text.trim() || loading}>
          {loading ? <><Loader2 className="w-4 h-4 animate-spin mr-1" /> Analyzing...</> : <><MessageSquare className="w-4 h-4 mr-1" /> Analyze Sentiment</>}
        </Button>
      </div>

      {result && (
        <div className={`p-4 rounded-xl animate-fade-in ${sentimentColors[result.sentiment]}`}>
          ...
          {result.keywords.length > 0 && (
            <div className="flex gap-2 flex-wrap mt-2">
              {result.keywords.map(k => (
                <span key={k} className="text-xs px-2 py-0.5 bg-background/50 rounded-full border border-current/20">{k}</span>
              ))}
            </div>
          )}
        </div>
