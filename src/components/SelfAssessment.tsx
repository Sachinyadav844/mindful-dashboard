...
const getRiskLevel = (score: number) => {
  ...
  if (score <= 9) return { label: 'Moderate Risk', color: 'risk-medium', emoji: '⚠️' };
...
const SelfAssessment = () => {
  ...
  if (submitted) {
    return (
      <div className="text-center py-8 animate-fade-in-up">
        ...
        <p className="text-muted-foreground mb-4">Assessment Score: <strong>{totalScore}/15</strong></p>
        <div className={cn('inline-block px-6 py-3 rounded-xl font-semibold', risk.color)}>
          {risk.label}
        </div>
        <div className="mt-6">
          <Button variant="outline" onClick={() => { setSubmitted(false); setAnswers(new Array(questions.length).fill(null)); }}>
            Retake Assessment
          </Button>
        </div>
      </div>
  ...
  return (
    <div className="space-y-6">
      {questions.map((q, qi) => (
        <div key={qi} className="space-y-3 animate-fade-in-up" style={{ animationDelay: `${qi * 80}ms` }}>
          <p className="font-medium text-sm">
            <span className="text-primary font-bold mr-2">Q{qi + 1}.</span>{q}
          </p>
      ...
      >
        {loading ? <><Loader2 className="w-4 h-4 animate-spin mr-2" /> Evaluating...</> : <><CheckCircle className="w-4 h-4 mr-2" /> Submit Assessment</>}
      </Button>
    </div>
  );
};
